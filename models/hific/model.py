...
class HiFiC(object):
  ...
  def _get_dataset(self, batch_size, crop_size, tfds_arguments: TFDSArguments):
    ...
    tf.logging.info("Building TFDS pipeline: %s, %s, %d, %d",
                    tfds_arguments, split, batch_size, crop_size)
    ...
    with tf.name_scope("tfds"):
      builder = tfds.builder(tfds_arguments.dataset_name,
                             data_dir=tfds_arguments.downloads_dir)
      ...
      def _preprocess(features):
        ...
        if not crop_size:
          return image
        tf.logging.info('Scaling down %s and cropping to %d x %d',
                        image, crop_size, crop_size)
        with tf.name_scope('random_scale'):
          # Scale down by at least `biggest_fac` and at most `smallest_fac` to remove
          # JPG artifacts. This code also handles images that have one side shorter
          # than crop_size. In this case, we always upscale such that this side
          # becomes the same as `crop_size`. Overall, images returned will never
          # be smaller than `crop_size`.
          image_shape = tf.cast(tf.shape(image), tf.float32)
          H, W = image_shape[0], image_shape[1]
          smallest_side = tf.math.minimum(H, W)
          # The smallest factor such that the downscaled image is still bigger than
          # `crop_size`. Will be bigger than 1 for images smaller than `crop_size`.
          ...
          scale = tf.random_uniform([], minval=min_fac, maxval=max_fac,
                                    dtype=tf.float32, seed=42, name=None)
          image = tf.image.resize_images(image, [tf.ceil(scale * H),
                                                 tf.ceil(scale * W)])
